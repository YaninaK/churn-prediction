...
* Random Forest обладает более высокой интерпретируемостью, чем нейросетевые модели, но его метрики существенно ниже.
* В качестве финальной модели предлагается нейросетевая модель с блоком LSTM, ембеддингами признаков state, occupation, education, объединенных с блоком признаков, отобранных в пункте 2.2.


### 2.4. Трансформация признаков
...
#### 2.4.1. Трансформация признаков, изменяющихся на заданном временном промежутке
...
* AUC на тестовой выборке 0.8055, что значительно выше, чем в базовом варианте Random Forest 0.7051.
...
* Модель хорошо срабатывает на клиентах старше 43 лет - AUC на тестовой выборке 0.9051, и больше ошибается на молодых клиентах - AUC 0.6775. 
* Чаще всего модель ошибочно классифицирует как отток (False Positive) клиенто старше 43 лет, как правило, женщин с numberofcomplaints больше 2, callfailurerate 0.02 и выше. 
* Модель затрудняется правильно определить отток (False Negative) у клентов моложе 40 лет, даже если numberofcomplaints больше 2 или callfailurerate_mean равна 0.02.

Notebook с анализом результатов инференса нейросетевой модели на тестовой выборке находится [по ссылке](https://github.com/YaninaK/churn-prediction/blob/main/notebooks/04_Inference_results_analysis.ipynb).
...
Можно увеличить длину последовательности. Например, не 3, а более месяцев. Таким образом, профиль клиента получится более объемный и есть шанс повысить точность модели.
...
Для развития модели имеет смысл подумать над дополнительными разделяющими признаками для клиентов моложе 43 лет или простроить дополнительную модель, которая бы работала с ошибкой или делала бы самостоятельные предсказания. 
...
Если в приоритете будет интерпретируемость, для объяснения можно использовать модели, апроксимирующие результат, хотя и обладающие меньшей точностью.
