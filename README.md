...
* Бустинг из General Linear Model (GLM) и Random Forest ожидаемо дал лучшие результаты чем базваая модель Random Forest.
* GLM модель обладает более высокой интерпретируемостью, но если метрики модели более важны, чем ее интерпретируемость, имеет смысл поэксперементировать с нейросетями.
...
* AUC на тестовой выборке 0.8055, что значительно выше, чем в базовом варианте Random Forest 0.7051 и в бустинге GLM + Random Forest - AUC: 0.742 +/- 0.0301
...
Для повышения точности, можно попробовать и другие методы ансамблирования.