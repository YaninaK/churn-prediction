...
* AUC на тестовой выборке 0.8054, что значительно выше, чем в базовом варианте Random Forest 0.7055
...
Модель имеет потенциал, AUC на тестовой выборке 0.8054 - неплохое начало.
...
Следует проанализировать, с какими группами клиентов модель работает лучше и сконцентрировалься на них, чтобы оптимизировать расходы на обработку False Positive результатов. 
...
Если в приоритете будет интерпретируемость, можно использовать бустинг, например, GLM + Random Forest. Линейная модель (GLM) поможет оценть факторы, влияяющие на результат, а Random Forest в бустинге уменьшит смещение модели и таким образом добавит точности. 
